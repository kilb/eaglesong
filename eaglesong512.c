...
#define SET(a)      _mm512_set1_epi32 ((a))
#define STORE       _mm512_store_epi32
#define SET16        _mm512_set_epi32
#define u512        __m512i
#define ZERO        _mm512_setzero_epi32()

#define ROL_ADD(a,b) a = ADD(a, b); a = ROL32(a, 8); b = ADD(ROL32(b, 24) ,a);
#define ROL_OR(t, a, b, k) XOR(XOR(XOR(t, ROL32(t, a)), ROL32(t, b)), SET(injection_constants[k]))
...
void EaglesongPermutation( u512 * state ) {
    u512 new[2][16];
    u512 tmp0, tmp1;
    int k = 0;
    ...
    for(int i = 0 ; i < ROUND ; ++i ) {
        ...
        tmp0 = XOR(XOR(XOR(pre[0],pre[4]),pre[12]),pre[15]);

        tmp1 = XOR(XOR(XOR(tmp0,pre[1]),pre[8]),pre[13]);
        tmp0 = XOR(XOR(XOR(tmp0,pre[5]),pre[6]),pre[7]);
        tmp0 = ROL_OR(tmp0, 2, 4, k); ++k;
        tmp1 = ROL_OR(tmp1, 13, 22, k); ++k;
        ROL_ADD(tmp0, tmp1); cur[0] = tmp0; cur[1] = tmp1;

        tmp0 = XOR(XOR(XOR(XOR(XOR(XOR(XOR(pre[0],pre[1]),pre[2]),pre[4]),pre[6]),pre[12]),pre[13]),pre[14]);
        tmp1 = XOR(XOR(XOR(tmp0,pre[3]),pre[8]),pre[10]);
        tmp0 = XOR(XOR(XOR(tmp0,pre[7]),pre[9]),pre[15]);
        tmp0 = ROL_OR(tmp0, 4, 19, k); ++k;
        tmp1 = ROL_OR(tmp1, 3, 14, k); ++k;
        ROL_ADD(tmp0, tmp1); cur[2] = tmp0; cur[3] = tmp1;

        tmp0 = XOR(XOR(XOR(pre[2],pre[3]),pre[7]),pre[14]);
        tmp1 = XOR(XOR(XOR(tmp0,pre[0]),pre[8]),pre[10]);
        tmp0 = XOR(XOR(XOR(XOR(XOR(XOR(XOR(tmp0,pre[1]),pre[4]),pre[5]),pre[9]),pre[11]),pre[13]),pre[15]);
        tmp0 = ROL_OR(tmp0, 27, 31, k); ++k;
        tmp1 = ROL_OR(tmp1, 3, 8, k); ++k;
        ROL_ADD(tmp0, tmp1); cur[4] = tmp0; cur[5] = tmp1;

        tmp0 = XOR(pre[9],pre[15]);
        tmp1 = XOR(XOR(XOR(XOR(XOR(tmp0,pre[0]),pre[2]),pre[6]),pre[7]),pre[10]);
        tmp0 = XOR(XOR(XOR(XOR(XOR(tmp0,pre[1]),pre[3]),pre[4]),pre[8]),pre[11]);
        tmp0 = ROL_OR(tmp0, 17, 26, k); ++k;
        tmp1 = ROL_OR(tmp1, 3, 12, k); ++k;
        ROL_ADD(tmp0, tmp1); cur[6] = tmp0; cur[7] = tmp1;

        tmp0 = XOR(XOR(XOR(pre[0],pre[1]),pre[11]),pre[15]);
        tmp1 = XOR(XOR(XOR(tmp0,pre[2]),pre[9]),pre[13]);
        tmp0 = XOR(XOR(XOR(XOR(XOR(XOR(XOR(tmp0, pre[3]),pre[4]),pre[5]),pre[6]),pre[8]),pre[10]),pre[12]);
        tmp0 = ROL_OR(tmp0, 18, 22, k); ++k;
        tmp1 = ROL_OR(tmp1, 12, 18, k); ++k;
        ROL_ADD(tmp0, tmp1); cur[8] = tmp0; cur[9] = tmp1;

        tmp0 = XOR(XOR(XOR(pre[0],pre[1]),pre[2]),pre[3]);
        tmp1 = XOR(XOR(XOR(tmp0,pre[8]),pre[11]),pre[12]);
        tmp0 = XOR(XOR(XOR(XOR(XOR(XOR(XOR(tmp0,pre[4]),pre[5]),pre[6]),pre[7]),pre[10]),pre[14]),pre[15]);
        tmp0 = ROL_OR(tmp0, 4, 7, k); ++k;
        tmp1 = ROL_OR(tmp1, 4, 31, k); ++k;
        ROL_ADD(tmp0, tmp1); cur[10] = tmp0; cur[11] = tmp1;

        tmp0 = XOR(XOR(XOR(pre[2],pre[3]),pre[4]),pre[13]);
        tmp1 = XOR(XOR(XOR(tmp0,pre[5]),pre[10]),pre[14]);
        tmp0 = XOR(XOR(XOR(tmp0,pre[1]),pre[9]),pre[12]);
        tmp0 = ROL_OR(tmp0, 12, 27, k); ++k;
        tmp1 = ROL_OR(tmp1, 7, 17, k); ++k;
        ROL_ADD(tmp0, tmp1); cur[12] = tmp0; cur[13] = tmp1;

        tmp0 = XOR(XOR(XOR(pre[3],pre[5]),pre[11]),pre[15]);
        tmp1 = XOR(XOR(XOR(XOR(XOR(XOR(XOR(tmp0,pre[0]),pre[1]),pre[2]),pre[7]),pre[8]),pre[9]),pre[10]);
        tmp0 = XOR(XOR(XOR(tmp0,pre[4]),pre[6]),pre[14]);
        tmp0 = ROL_OR(tmp0, 7, 8, k); ++k;
        tmp1 = ROL_OR(tmp1, 1, 13, k); ++k;
        ROL_ADD(tmp0, tmp1); cur[14] = tmp0; cur[15] = tmp1;
    }

    for(int i=0; i<16; ++i) {
        state[i] = new[ROUND&1^0][i];
    }
}

void EaglesongHash(uint8_t output[16][32], uint8_t **input, uint32_t input_length, uint32_t *ans) {
    u512 state[16];
    uint32_t integer[16];
    ...
    for(int i = 0, k = 0; i < n; ++i) {
        for(int j = 0 ; j < M; ++j) {
            ...
            k+=4;
            state[j] = XOR(state[j], SET16(integer[15],integer[14],integer[13],integer[12],integer[11],integer[10],integer[9],integer[8], \
                                          integer[7],integer[6],integer[5],integer[4],integer[3],integer[2],integer[1],integer[0]));
        }
    ...
    for(int i = 0, k = 0; ;) {
        for(int j = 0 ; j < M; ++j) {
            STORE(ans, state[j]);
            for(int u=0; u < 16; ++u) {
                ((uint32_t *)output[u])[k] = htole32(ans[u]);
            }
            ++k;
        }
...
int main() {
    unsigned char str[100] = "Hello, world, bcdf, ghc, dfd, efg, ggc, df ijiefk,ijoajidjfojefjwoj!\n";
    uint8_t* input[16];
    uint8_t output[16][32];

    uint32_t *ans = _mm_malloc(sizeof(uint32_t) * 16, 64);
	...
	t1 = clock();
	for (int i = 0; i < sum; ++i)EaglesongHash(output, input, 8, ans);
	t2 = clock();
    // for (int i = 0; i < 32; ++i) printf("%02x", output[i]); printf("\n");
    printf("time: %f  %f ms\n", (double)(t2 - t1) / CLOCKS_PER_SEC, (double)(sum) / ((double)(t2 - t1) / (CLOCKS_PER_SEC)));
    for (int i = 0; i < 32; ++i) printf("%02x", output[0][i]);printf("\n");
    for (int i = 0; i < 32; ++i) printf("%02x", output[1][i]);printf("\n");
}
