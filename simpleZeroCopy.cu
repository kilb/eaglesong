...
__global__ void eaglesong(uint8_t *chain_block, uint32_t blacklen, uint32_t salt, uint32_t *gsalt_out, uint8_t* target)
{
	...
	uint32_t temp0, temp1, i, j, k, hashblock, integer, blockid,flag;

	__shared__ uint32_t state2[threadsPerBlock][(Hash_Capacity + Hash_Rate) >> 5];
	__shared__ uint32_t state[threadsPerBlock][(Hash_Capacity + Hash_Rate) >> 5];
	__shared__ uint8_t chain_block_Byte[MaxMesageLen];
	__shared__ uint8_t target_Byte[Hash_Len];

	memset(state[block_tid], 0, sizeof(uint32_t)*((Hash_Capacity + Hash_Rate) >> 5));

	if (block_tid == 0){memcpy(chain_block_Byte, chain_block, blacklen); memcpy(target_Byte, target, Hash_Len);}
	__syncthreads();
	salt = salt + id;
	for (blockid = 0; blockid < ((blacklen + 1) * 8 + Hash_Rate - 1) / Hash_Rate; ++blockid) //循环结构疑似过于繁琐
	{
		...
		for (j = 0; j < Hash_Rate / 32; ++j)
		{
			...
			for (k = 0; k < 4; ++k)
			{
				if (blockid*Hash_Rate / 8 + j * 4 + k < blacklen)
				{
					//salt??????????????????????????????????????????????????? 这里必须想办法插入盐
					if (blockid == 0 && j == 0)
						integer = (integer << 8) ^ ((salt >> (8 * k)) & 0xff) ^ chain_block_Byte[blockid*(Hash_Rate / 8) + j * 4 + k];
					else
						integer = (integer << 8) ^ chain_block_Byte[blockid*(Hash_Rate / 8) + j * 4 + k];

				}
				else if (blockid*Hash_Rate / 8 + j * 4 + k == blacklen)
				{
					integer = (integer << 8) ^ 0x06;
				}
			}
			state[block_tid][j] = state[block_tid][j] ^ integer;
		}
		//EaglesongPermutation
		for (i = 0; i < Hash_Rounds; ++i)
		{
			temp0 = 0;
			temp0 = temp0 ^ state[block_tid][0];
			temp0 = temp0 ^ state[block_tid][4];
			temp0 = temp0 ^ state[block_tid][5];
			temp0 = temp0 ^ state[block_tid][6];
			temp0 = temp0 ^ state[block_tid][7];
			temp0 = temp0 ^ state[block_tid][12];
			temp0 = temp0 ^ state[block_tid][15];
			temp0 = temp0 ^ ROL32(temp0, 2) ^ ROL32(temp0, 4) ^ gpu_injection_constants[i * 16 + 0];

			temp1 = 0;
			temp1 = temp1 ^ state[block_tid][0];
			temp1 = temp1 ^ state[block_tid][1];
			temp1 = temp1 ^ state[block_tid][4];
			temp1 = temp1 ^ state[block_tid][8];
			temp1 = temp1 ^ state[block_tid][12];
			temp1 = temp1 ^ state[block_tid][13];
			temp1 = temp1 ^ state[block_tid][15];
			temp1 = temp1 ^ ROL32(temp1, 13) ^ ROL32(temp1, 22) ^ gpu_injection_constants[i * 16 + 1];
			temp0 = temp0 + temp1;
			temp0 = ROL32(temp0, 8);
			state2[block_tid][1 - 1] = temp0;
			temp1 = ROL32(temp1, 24);
			state2[block_tid][1] = temp0 + temp1;

			temp0 = 0;
			temp0 = temp0 ^ state[block_tid][0];
			temp0 = temp0 ^ state[block_tid][1];
			temp0 = temp0 ^ state[block_tid][2];
			temp0 = temp0 ^ state[block_tid][4];
			temp0 = temp0 ^ state[block_tid][6];
			temp0 = temp0 ^ state[block_tid][7];
			temp0 = temp0 ^ state[block_tid][9];
			temp0 = temp0 ^ state[block_tid][12];
			temp0 = temp0 ^ state[block_tid][13];
			temp0 = temp0 ^ state[block_tid][14];
			temp0 = temp0 ^ state[block_tid][15];
			temp0 = temp0 ^ ROL32(temp0, 4) ^ ROL32(temp0, 19) ^ gpu_injection_constants[i * 16 + 2];

			temp1 = 0;
			temp1 = temp1 ^ state[block_tid][0];
			temp1 = temp1 ^ state[block_tid][1];
			temp1 = temp1 ^ state[block_tid][2];
			temp1 = temp1 ^ state[block_tid][3];
			temp1 = temp1 ^ state[block_tid][4];
			temp1 = temp1 ^ state[block_tid][6];
			temp1 = temp1 ^ state[block_tid][8];
			temp1 = temp1 ^ state[block_tid][10];
			temp1 = temp1 ^ state[block_tid][12];
			temp1 = temp1 ^ state[block_tid][13];
			temp1 = temp1 ^ state[block_tid][14];
			temp1 = temp1 ^ ROL32(temp1, 3) ^ ROL32(temp1, 14) ^ gpu_injection_constants[i * 16 + 3];
			temp0 = temp0 + temp1;
			temp0 = ROL32(temp0, 8);
			state2[block_tid][3 - 1] = temp0;
			temp1 = ROL32(temp1, 24);
			state2[block_tid][3] = temp0 + temp1;


			temp0 = 0;
			temp0 = temp0 ^ state[block_tid][1];
			temp0 = temp0 ^ state[block_tid][2];
			temp0 = temp0 ^ state[block_tid][3];
			temp0 = temp0 ^ state[block_tid][4];
			temp0 = temp0 ^ state[block_tid][5];
			temp0 = temp0 ^ state[block_tid][7];
			temp0 = temp0 ^ state[block_tid][9];
			temp0 = temp0 ^ state[block_tid][11];
			temp0 = temp0 ^ state[block_tid][13];
			temp0 = temp0 ^ state[block_tid][14];
			temp0 = temp0 ^ state[block_tid][15];
			temp0 = temp0 ^ ROL32(temp0, 27) ^ ROL32(temp0, 31) ^ gpu_injection_constants[i * 16 + 4];

			temp1 = 0;
			temp1 = temp1 ^ state[block_tid][0];
			temp1 = temp1 ^ state[block_tid][2];
			temp1 = temp1 ^ state[block_tid][3];
			temp1 = temp1 ^ state[block_tid][7];
			temp1 = temp1 ^ state[block_tid][8];
			temp1 = temp1 ^ state[block_tid][10];
			temp1 = temp1 ^ state[block_tid][14];
			temp1 = temp1 ^ ROL32(temp1, 3) ^ ROL32(temp1, 8) ^ gpu_injection_constants[i * 16 + 5];
			temp0 = temp0 + temp1;
			temp0 = ROL32(temp0, 8);
			state2[block_tid][5 - 1] = temp0;
			temp1 = ROL32(temp1, 24);
			state2[block_tid][5] = temp0 + temp1;
			...
			temp0 = temp0 ^ state[block_tid][3];
			temp0 = temp0 ^ state[block_tid][4];
			temp0 = temp0 ^ state[block_tid][8];
			temp0 = temp0 ^ state[block_tid][9];
			temp0 = temp0 ^ state[block_tid][11];
			temp0 = temp0 ^ state[block_tid][15];
			temp0 = temp0 ^ ROL32(temp0, 17) ^ ROL32(temp0, 26) ^ gpu_injection_constants[i * 16 + 6];
			...
			temp1 = temp1 ^ state[block_tid][6];
			temp1 = temp1 ^ state[block_tid][7];
			temp1 = temp1 ^ state[block_tid][9];
			temp1 = temp1 ^ state[block_tid][10];
			temp1 = temp1 ^ state[block_tid][15];
			temp1 = temp1 ^ ROL32(temp1, 3) ^ ROL32(temp1, 12) ^ gpu_injection_constants[i * 16 + 7];
			temp0 = temp0 + temp1;
			temp0 = ROL32(temp0, 8);
			state2[block_tid][7 - 1] = temp0;
			temp1 = ROL32(temp1, 24);
			state2[block_tid][7] = temp0 + temp1;
			...
			temp0 = temp0 ^ state[block_tid][6];
			temp0 = temp0 ^ state[block_tid][8];
			temp0 = temp0 ^ state[block_tid][10];
			temp0 = temp0 ^ state[block_tid][11];
			temp0 = temp0 ^ state[block_tid][12];
			temp0 = temp0 ^ state[block_tid][15];
			temp0 = temp0 ^ ROL32(temp0, 18) ^ ROL32(temp0, 22) ^ gpu_injection_constants[i * 16 + 8];
			...
			temp1 = temp1 ^ state[block_tid][2];
			temp1 = temp1 ^ state[block_tid][9];
			temp1 = temp1 ^ state[block_tid][11];
			temp1 = temp1 ^ state[block_tid][13];
			temp1 = temp1 ^ state[block_tid][15];
			temp1 = temp1 ^ ROL32(temp1, 12) ^ ROL32(temp1, 18) ^ gpu_injection_constants[i * 16 + 9];
			temp0 = temp0 + temp1;
			temp0 = ROL32(temp0, 8);
			state2[block_tid][9 - 1] = temp0;
			temp1 = ROL32(temp1, 24);
			state2[block_tid][9] = temp0 + temp1;
			...
			temp0 = temp0 ^ state[block_tid][14];
			temp0 = temp0 ^ state[block_tid][15];
			temp0 = temp0 ^ ROL32(temp0, 4) ^ ROL32(temp0, 7) ^ gpu_injection_constants[i * 16 + 10];
			...
			temp1 = temp1 ^ state[block_tid][11];
			temp1 = temp1 ^ state[block_tid][12];
			temp1 = temp1 ^ ROL32(temp1, 4) ^ ROL32(temp1, 31) ^ gpu_injection_constants[i * 16 + 11];
			temp0 = temp0 + temp1;
			temp0 = ROL32(temp0, 8);
			state2[block_tid][11 - 1] = temp0;
			temp1 = ROL32(temp1, 24);
			state2[block_tid][11] = temp0 + temp1;
			...
			temp0 = temp0 ^ state[block_tid][12];
			temp0 = temp0 ^ state[block_tid][13];
			temp0 = temp0 ^ ROL32(temp0, 12) ^ ROL32(temp0, 27) ^ gpu_injection_constants[i * 16 + 12];

			temp1 = 0;
			temp1 = temp1 ^ state[block_tid][2];
			temp1 = temp1 ^ state[block_tid][3];
			temp1 = temp1 ^ state[block_tid][4];
			temp1 = temp1 ^ state[block_tid][5];
			temp1 = temp1 ^ state[block_tid][10];
			temp1 = temp1 ^ state[block_tid][13];
			temp1 = temp1 ^ state[block_tid][14];
			temp1 = temp1 ^ ROL32(temp1, 7) ^ ROL32(temp1, 17) ^ gpu_injection_constants[i * 16 + 13];
			temp0 = temp0 + temp1;
			temp0 = ROL32(temp0, 8);
			state2[block_tid][13 - 1] = temp0;
			temp1 = ROL32(temp1, 24);
			state2[block_tid][13] = temp0 + temp1;
			...
			temp0 = temp0 ^ state[block_tid][14];
			temp0 = temp0 ^ state[block_tid][15];
			temp0 = temp0 ^ ROL32(temp0, 7) ^ ROL32(temp0, 8) ^ gpu_injection_constants[i * 16 + 14];
			...
			temp1 = temp1 ^ state[block_tid][11];
			temp1 = temp1 ^ state[block_tid][15];
			temp1 = temp1 ^ ROL32(temp1, 1) ^ ROL32(temp1, 13) ^ gpu_injection_constants[i * 16 + 15];
			temp0 = temp0 + temp1;
			temp0 = ROL32(temp0, 8);
			state2[block_tid][15 - 1] = temp0;
			temp1 = ROL32(temp1, 24);
			state2[block_tid][15] = temp0 + temp1;

			i++;
			if (i >= Hash_Rounds)break;

			temp0 = 0;
			temp0 = temp0 ^ state2[block_tid][0];
			temp0 = temp0 ^ state2[block_tid][4];
			temp0 = temp0 ^ state2[block_tid][5];
			temp0 = temp0 ^ state2[block_tid][6];
			temp0 = temp0 ^ state2[block_tid][7];
			temp0 = temp0 ^ state2[block_tid][12];
			temp0 = temp0 ^ state2[block_tid][15];
			temp0 = temp0 ^ ROL32(temp0, 2) ^ ROL32(temp0, 4) ^ gpu_injection_constants[i * 16 + 0];

			temp1 = 0;
			temp1 = temp1 ^ state2[block_tid][0];
			temp1 = temp1 ^ state2[block_tid][1];
			temp1 = temp1 ^ state2[block_tid][4];
			temp1 = temp1 ^ state2[block_tid][8];
			temp1 = temp1 ^ state2[block_tid][12];
			temp1 = temp1 ^ state2[block_tid][13];
			temp1 = temp1 ^ state2[block_tid][15];
			temp1 = temp1 ^ ROL32(temp1, 13) ^ ROL32(temp1, 22) ^ gpu_injection_constants[i * 16 + 1];
			temp0 = temp0 + temp1;
			temp0 = ROL32(temp0, 8);
			state[block_tid][1 - 1] = temp0;
			temp1 = ROL32(temp1, 24);
			state[block_tid][1] = temp0 + temp1;

			temp0 = 0;
			temp0 = temp0 ^ state2[block_tid][0];
			temp0 = temp0 ^ state2[block_tid][1];
			temp0 = temp0 ^ state2[block_tid][2];
			temp0 = temp0 ^ state2[block_tid][4];
			temp0 = temp0 ^ state2[block_tid][6];
			temp0 = temp0 ^ state2[block_tid][7];
			temp0 = temp0 ^ state2[block_tid][9];
			temp0 = temp0 ^ state2[block_tid][12];
			temp0 = temp0 ^ state2[block_tid][13];
			temp0 = temp0 ^ state2[block_tid][14];
			temp0 = temp0 ^ state2[block_tid][15];
			temp0 = temp0 ^ ROL32(temp0, 4) ^ ROL32(temp0, 19) ^ gpu_injection_constants[i * 16 + 2];
			...
			temp1 = temp1 ^ state2[block_tid][0];
			temp1 = temp1 ^ state2[block_tid][1];
			temp1 = temp1 ^ state2[block_tid][2];
			temp1 = temp1 ^ state2[block_tid][3];
			temp1 = temp1 ^ state2[block_tid][4];
			temp1 = temp1 ^ state2[block_tid][6];
			temp1 = temp1 ^ state2[block_tid][8];
			temp1 = temp1 ^ state2[block_tid][10];
			temp1 = temp1 ^ state2[block_tid][12];
			temp1 = temp1 ^ state2[block_tid][13];
			temp1 = temp1 ^ state2[block_tid][14];
			temp1 = temp1 ^ ROL32(temp1, 3) ^ ROL32(temp1, 14) ^ gpu_injection_constants[i * 16 + 3];
			temp0 = temp0 + temp1;
			temp0 = ROL32(temp0, 8);
			state[block_tid][3 - 1] = temp0;
			temp1 = ROL32(temp1, 24);
			state[block_tid][3] = temp0 + temp1;


			temp0 = 0;
			temp0 = temp0 ^ state2[block_tid][1];
			temp0 = temp0 ^ state2[block_tid][2];
			temp0 = temp0 ^ state2[block_tid][3];
			temp0 = temp0 ^ state2[block_tid][4];
			temp0 = temp0 ^ state2[block_tid][5];
			temp0 = temp0 ^ state2[block_tid][7];
			temp0 = temp0 ^ state2[block_tid][9];
			temp0 = temp0 ^ state2[block_tid][11];
			temp0 = temp0 ^ state2[block_tid][13];
			temp0 = temp0 ^ state2[block_tid][14];
			temp0 = temp0 ^ state2[block_tid][15];
			temp0 = temp0 ^ ROL32(temp0, 27) ^ ROL32(temp0, 31) ^ gpu_injection_constants[i * 16 + 4];
			...
			temp1 = temp1 ^ state2[block_tid][2];
			temp1 = temp1 ^ state2[block_tid][3];
			temp1 = temp1 ^ state2[block_tid][7];
			temp1 = temp1 ^ state2[block_tid][8];
			temp1 = temp1 ^ state2[block_tid][10];
			temp1 = temp1 ^ state2[block_tid][14];
			temp1 = temp1 ^ ROL32(temp1, 3) ^ ROL32(temp1, 8) ^ gpu_injection_constants[i * 16 + 5];
			temp0 = temp0 + temp1;
			temp0 = ROL32(temp0, 8);
			state[block_tid][5 - 1] = temp0;
			temp1 = ROL32(temp1, 24);
			state[block_tid][5] = temp0 + temp1;
			...
			temp0 = temp0 ^ state2[block_tid][3];
			temp0 = temp0 ^ state2[block_tid][4];
			temp0 = temp0 ^ state2[block_tid][8];
			temp0 = temp0 ^ state2[block_tid][9];
			temp0 = temp0 ^ state2[block_tid][11];
			temp0 = temp0 ^ state2[block_tid][15];
			temp0 = temp0 ^ ROL32(temp0, 17) ^ ROL32(temp0, 26) ^ gpu_injection_constants[i * 16 + 6];
			...
			temp1 = temp1 ^ state2[block_tid][6];
			temp1 = temp1 ^ state2[block_tid][7];
			temp1 = temp1 ^ state2[block_tid][9];
			temp1 = temp1 ^ state2[block_tid][10];
			temp1 = temp1 ^ state2[block_tid][15];
			temp1 = temp1 ^ ROL32(temp1, 3) ^ ROL32(temp1, 12) ^ gpu_injection_constants[i * 16 + 7];
			temp0 = temp0 + temp1;
			temp0 = ROL32(temp0, 8);
			state[block_tid][7 - 1] = temp0;
			temp1 = ROL32(temp1, 24);
			state[block_tid][7] = temp0 + temp1;
			...
			temp0 = temp0 ^ state2[block_tid][6];
			temp0 = temp0 ^ state2[block_tid][8];
			temp0 = temp0 ^ state2[block_tid][10];
			temp0 = temp0 ^ state2[block_tid][11];
			temp0 = temp0 ^ state2[block_tid][12];
			temp0 = temp0 ^ state2[block_tid][15];
			temp0 = temp0 ^ ROL32(temp0, 18) ^ ROL32(temp0, 22) ^ gpu_injection_constants[i * 16 + 8];
			...
			temp1 = temp1 ^ state2[block_tid][2];
			temp1 = temp1 ^ state2[block_tid][9];
			temp1 = temp1 ^ state2[block_tid][11];
			temp1 = temp1 ^ state2[block_tid][13];
			temp1 = temp1 ^ state2[block_tid][15];
			temp1 = temp1 ^ ROL32(temp1, 12) ^ ROL32(temp1, 18) ^ gpu_injection_constants[i * 16 + 9];
			temp0 = temp0 + temp1;
			temp0 = ROL32(temp0, 8);
			state[block_tid][9 - 1] = temp0;
			temp1 = ROL32(temp1, 24);
			state[block_tid][9] = temp0 + temp1;
			...
			temp0 = temp0 + temp1;
			temp0 = ROL32(temp0, 8);
			state[block_tid][11 - 1] = temp0;
			temp1 = ROL32(temp1, 24);
			state[block_tid][11] = temp0 + temp1;
			...
			temp1 = temp1 ^ state2[block_tid][3];
			temp1 = temp1 ^ state2[block_tid][4];
			temp1 = temp1 ^ state2[block_tid][5];
			temp1 = temp1 ^ state2[block_tid][10];
			temp1 = temp1 ^ state2[block_tid][13];
			temp1 = temp1 ^ state2[block_tid][14];
			temp1 = temp1 ^ ROL32(temp1, 7) ^ ROL32(temp1, 17) ^ gpu_injection_constants[i * 16 + 13];
			temp0 = temp0 + temp1;
			temp0 = ROL32(temp0, 8);
			state[block_tid][13 - 1] = temp0;
			temp1 = ROL32(temp1, 24);
			state[block_tid][13] = temp0 + temp1;
			...
			temp0 = temp0 + temp1;
			temp0 = ROL32(temp0, 8);
			state[block_tid][15 - 1] = temp0;
			temp1 = ROL32(temp1, 24);
			state[block_tid][15] = temp0 + temp1;
		}
...
//只有奇数轮可以这么写
		...
		for (j = 0; j < Hash_Rate / 32; ++j)
		{
			for (i = 0; i < 4; ++i)
			{
				if (target_Byte[j * 4 + i]>((state2[block_tid][j] >> (8 * i))&0xff))
				{
					flag = 1;
					break;
				}
				...
				{
					if (target_Byte[j * 4 + i] < ((state2[block_tid][j] >> (8 * i)) & 0xff))break;
				}
			}
			if (i != 4)break;
		}
...
int gpu_hash(uint32_t gpuid)
{
	...
	eaglesong << <HashNum / threadsPerBlock, threadsPerBlock >> >(gpu_divices[gpuid]->gchain_block, 14, gpu_divices[gpuid]->salt_in, gpu_divices[gpuid]->gsalt_out, gpu_divices[gpuid]->gtarget);
...
int main(int argc, char ** argv)
{
	...
	t1 = clock();
	for (i = 0; i < sum; ++i)EaglesongHash(output, input, 14,0);
	t2 = clock();

	printf("time: %f  %f per sec\n", (double)(t2 - t1) / CLOCKS_PER_SEC, (double)(sum) / ((double)(t2 - t1) / (CLOCKS_PER_SEC)));
	for (i = 0; i < 32; ++i)
	{
		printf("%02x", output[i]);
	}
	...
	for (i = 0; i < gpu_divices_cnt;i++)
	{
		...
		gpu_divices[i]->target[3] = 0x1f;
		gpu_divices[i]->salt_in = 0;
		salt = gpu_hash(i);
	}

	EaglesongHash(output, input, 14, salt);
	for (i = 0; i < 32; ++i)
	{
		printf("%02x", output[i]);
	}
