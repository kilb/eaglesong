...
void EaglesongPermutation( uint32_t * state ) {
    ...
    memcpy(new[0], state, 16<<2);

    for( i = 0 ; i < ROUND ; ++i ) {
        uint32_t *pre = new[i&1^0];
        uint32_t *cur = new[i&1^1];

        tmp0 = pre[0]^pre[4]^pre[5]^pre[6]^pre[7]^pre[12]^pre[15];
        tmp1 = pre[0]^pre[1]^pre[4]^pre[8]^pre[12]^pre[13]^pre[15];
        tmp0 ^= ROL32(tmp0, 2)^ROL32(tmp0, 4)^injection_constants[k]; ++k;
        tmp1 ^= ROL32(tmp1, 13)^ROL32(tmp1, 22)^injection_constants[k]; ++k;
        ROL_ADD(tmp0, tmp1); cur[0] = tmp0; cur[1] = tmp1;

        tmp0 = pre[0]^pre[1]^pre[2]^pre[4]^pre[6]^pre[7]^pre[9]^pre[12]^pre[13]^pre[14]^pre[15];
        tmp1 = pre[0]^pre[1]^pre[2]^pre[3]^pre[4]^pre[6]^pre[8]^pre[10]^pre[12]^pre[13]^pre[14];
        tmp0 ^= ROL32(tmp0, 4)^ROL32(tmp0, 19)^injection_constants[k]; ++k;
        tmp1 ^= ROL32(tmp1, 3)^ROL32(tmp1, 14)^injection_constants[k]; ++k;
        ROL_ADD(tmp0, tmp1); cur[2] = tmp0; cur[3] = tmp1;

        tmp0 = pre[1]^pre[2]^pre[3]^pre[4]^pre[5]^pre[7]^pre[9]^pre[11]^pre[13]^pre[14]^pre[15];
        tmp1 = pre[0]^pre[2]^pre[3]^pre[7]^pre[8]^pre[10]^pre[14];
        tmp0 ^= ROL32(tmp0, 27)^ROL32(tmp0, 31)^injection_constants[k]; ++k;
        tmp1 ^= ROL32(tmp1, 3)^ROL32(tmp1, 8)^injection_constants[k]; ++k;
        ROL_ADD(tmp0, tmp1); cur[4] = tmp0; cur[5] = tmp1;

        tmp0 = pre[1]^pre[3]^pre[4]^pre[8]^pre[9]^pre[11]^pre[15];
        tmp1 = pre[0]^pre[2]^pre[6]^pre[7]^pre[9]^pre[10]^pre[15];
        tmp0 ^= ROL32(tmp0, 17)^ROL32(tmp0, 26)^injection_constants[k]; ++k;
        tmp1 ^= ROL32(tmp1, 3)^ROL32(tmp1, 12)^injection_constants[k]; ++k;
        ROL_ADD(tmp0, tmp1); cur[6] = tmp0; cur[7] = tmp1;

        tmp0 = pre[0]^pre[1]^pre[3]^pre[4]^pre[5]^pre[6]^pre[8]^pre[10]^pre[11]^pre[12]^pre[15];
        tmp1 = pre[0]^pre[1]^pre[2]^pre[9]^pre[11]^pre[13]^pre[15];
        tmp0 ^= ROL32(tmp0, 18)^ROL32(tmp0, 22)^injection_constants[k]; ++k;
        tmp1 ^= ROL32(tmp1, 12)^ROL32(tmp1, 18)^injection_constants[k]; ++k;
        ROL_ADD(tmp0, tmp1); cur[8] = tmp0; cur[9] = tmp1;

        tmp0 = pre[0]^pre[1]^pre[2]^pre[3];
        tmp1 = tmp0^pre[8]^pre[11]^pre[12];
        tmp0 ^= pre[4]^pre[5]^pre[6]^pre[7]^pre[10]^pre[14]^pre[15];
        tmp0 ^= ROL32(tmp0, 4)^ROL32(tmp0, 7)^injection_constants[k]; ++k;
        tmp1 ^= ROL32(tmp1, 4)^ROL32(tmp1, 31)^injection_constants[k]; ++k;
        ROL_ADD(tmp0, tmp1); cur[10] = tmp0; cur[11] = tmp1;

        tmp0 = pre[2]^pre[3]^pre[4]^pre[13];
        tmp1 = tmp0^pre[5]^pre[10]^pre[14];
        tmp0 ^= pre[1]^pre[9]^pre[12];
        tmp0 ^= ROL32(tmp0, 12)^ROL32(tmp0, 27)^injection_constants[k]; ++k;
        tmp1 ^= ROL32(tmp1, 7)^ROL32(tmp1, 17)^injection_constants[k]; ++k;
        ROL_ADD(tmp0, tmp1); cur[12] = tmp0; cur[13] = tmp1;

        tmp0 = pre[3]^pre[5]^pre[11]^pre[15];
        tmp1 = tmp0^pre[0]^pre[1]^pre[2]^pre[7]^pre[8]^pre[9]^pre[10];
        tmp0 ^= pre[4]^pre[6]^pre[14];
        tmp0 ^= ROL32(tmp0, 7)^ROL32(tmp0, 8)^injection_constants[k]; ++k;
        tmp1 ^= ROL32(tmp1, 1)^ROL32(tmp1, 13)^injection_constants[k]; ++k;
        ROL_ADD(tmp0, tmp1); cur[14] = tmp0; cur[15] = tmp1;
    }

    memcpy(state, new[ROUND&1^0], 16<<2);
}

void EaglesongHash(uint8_t *output, uint8_t *input, uint32_t input_length) {
    uint32_t state[16];
    uint32_t integer;
...
int main() {
    unsigned char input[] = "Hello, world, bcdf, ghc, dfd, efg, ggc, df ijiefk,ijoajidjfojefjwoj!\n";
    uint8_t output[32];
	...
	t1 = clock();
	for (int i = 0; i < sum; ++i)EaglesongHash(output, input, 60);
	t2 = clock();
