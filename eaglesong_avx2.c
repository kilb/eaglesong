...
#define ROL_XOR(t, a, b, k) XOR(XOR(XOR(t, ROL32(t, a)), ROL32(t, b)), SET1(injection_constants[k]))
...
uint32_t EaglesongHash(uint8_t *input, uint8_t *target, uint64_t *nonce) {
...
int main() {
	...
	t1 = clock();
	int sum = EaglesongHash(input, target, &nonce);
	t2 = clock();
